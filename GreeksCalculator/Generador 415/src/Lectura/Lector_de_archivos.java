...
//Renumerador ren = new Renumerador();
	...
	public static void main(String[] args) throws FileNotFoundException, ParseException {
		...
		String fechaHeader=null;
		String fechaFormato=null;
		String Dia = null;
        String Mes = null;
		Calendar fecha = Calendar.getInstance();
		//fecha.add(Calendar.DAY_OF_MONTH, 1);//borrar
		if(fecha.get(Calendar.DAY_OF_WEEK)== Calendar.MONDAY) {
	        ...
	        String[] festivoValidar=leerMx.LeerMX2("COL "+año, ".\\Generador 415\\Archivos\\Murex3\\Festivos_2022-2030.txt");
	        if(dia>= 1 && dia<10) {
	        	Dia = "0"+dia;
	        }else {
	        	Dia = ""+dia;
	        ...
	        fechaHeader=Dia+"-"+Mes+"-"+año+"_";
	         ...
	         for(int i=0;i<festivoValidar.length;i++) {
	         if(festivoValidar[i].equals(fechaActual)) {
	        	 fecha.add(Calendar.DAY_OF_MONTH, -1);
	        	 //diasRestar = diasRestar + 1;
	        	 System.out.println(festivoValidar[i] + " "+  fechaActual);
	        	 i = festivoValidar.length;
	         }
	         else if(!festivoValidar[i].equals(fechaActual)&& i==festivoValidar.length-1) {
	        	 habil = true;
...
public static void LeerHeaderOpti(String fechaHeader, String fechaFormato, String fechaActual)throws FileNotFoundException, ParseException{
    ...
    try {
        ...
        bufferedWriter = new BufferedWriter(new FileWriter("\\\\82.0.54.124\\c$\\Users\\c782970\\Desktop\\FORMATO415_"+fechaFormato+".csv"));//\\\\82.0.54.124\\c$\\Users\\c782970\\Desktop\\FORMATO415_"+fechaFormato+".csv"
        ...
        while(lineaRenum != null) {
        	 ...
        	 NoBoletaMx3 = fields[0];  	 
           	 String[] infoMx3=leerMx.LeerMX2(NoBoletaMx3,"\\\\co.igrupobbva\\svrfilesystem\\tx\\envio_host\\DIALOGO_MUREX\\"+fechaHeader+"dialogo_header_opti.txt");//\\\\co.igrupobbva\\svrfilesystem\\tx\\envio_host\\DIALOGO_MUREX\\"+fechaHeader+"dialogo_header_opti.txt
        	 ...
        	 if(infoMx3 != null && Double.parseDouble(infoMx3[21])>0) {
                 ...
                 if(fechaHoy.get(Calendar.DAY_OF_WEEK)== Calendar.FRIDAY) {
                	 fechaT2 = fechaHoy; 
                	 fechaT2.add(Calendar.DAY_OF_MONTH, 3);
                	 int habil=0;
                	 while(habil<2) {
         	        ...
         	        String[] festivoValidarCOL=leerMx.LeerMX2("COL "+anio, ".\\Generador 415\\Archivos\\Murex3\\Festivos_2022-2030.txt");
         	       String[] festivoValidarNYC=leerMx.LeerMX2("NYC "+anio, ".\\Generador 415\\Archivos\\Murex3\\Festivos_2022-2030.txt");
                	String[] festivosTotales = new String[festivoValidarCOL.length + festivoValidarNYC.length]; 
                	System.arraycopy(festivoValidarCOL, 0, festivosTotales, 0, festivoValidarCOL.length);
                	System.arraycopy(festivoValidarNYC, 0, festivosTotales, festivoValidarCOL.length, festivoValidarNYC.length);
         	       String FechaProcT2 = Dia+"/"+Mes+"/"+anio;
                	 for(int i=0;i<festivosTotales.length;i++) {
            	         if(festivosTotales[i].equals(FechaProcT2)) {
            	        	 fechaT2.add(Calendar.DAY_OF_MONTH, 1);
            	        	 //diasRestar = diasRestar + 1;
            	        	 System.out.println(festivosTotales[i] + " "+  fechaHoy);
            	        	 i = festivosTotales.length;
            	         }
            	         else if(!festivosTotales[i].equals(FechaProcT2)&& i==festivosTotales.length-1) {
            	        	 if(habil ==0){
                	        	 fechaT2.add(Calendar.DAY_OF_MONTH, 1);
                	        	 }
            	        	 habil = habil+1;
                	 ...
                	 fecha_t2_dias_proceso = sdf.parse(FechaProcT2);
             	       ...
             	       String FechaProcT2 = Dia+"/"+Mes+"/"+anio;
                    	 for(int i=0;i<festivosTotales.length;i++) {
                	         if(festivosTotales[i].equals(FechaProcT2)) {
                	        	 if(fechaT2.get(Calendar.DAY_OF_WEEK)==Calendar.FRIDAY) {
                	        		 fechaT2.add(Calendar.DAY_OF_MONTH, 3);
                	        	 }else {
                	        	 fechaT2.add(Calendar.DAY_OF_MONTH, 1);}
                	        	 //diasRestar = diasRestar + 1;
                	        	 ...
                	        	 if(fechaT2.get(Calendar.DAY_OF_WEEK)==Calendar.FRIDAY&&habil==0) {
                	        		 fechaT2.add(Calendar.DAY_OF_MONTH, 3);
                	        	 }else if(habil ==0){
                	        	 fechaT2.add(Calendar.DAY_OF_MONTH, 1);
                	        	 }
                 ...
                 if(fechaVencimiento.get(Calendar.DAY_OF_WEEK)== Calendar.FRIDAY) {
                	 fechaVencimientoT2 = fechaVencimiento; 
                	 fechaVencimientoT2.add(Calendar.DAY_OF_MONTH, 3);
                	 int habil=0;
                	 while(habil<2) {
                	 ...
                	 for(int i=0;i<festivosTotales.length;i++) {
            	         if(festivosTotales[i].equals(FechaProcT2)) {
            	        	 fechaVencimientoT2.add(Calendar.DAY_OF_MONTH, 1);
            	        	 //diasRestar = diasRestar + 1;
            	        	 System.out.println(festivosTotales[i] + " "+  fechaHoy);
            	        	 i = festivosTotales.length;
            	         }
                	 ...
                	 for(int i=0;i<festivosTotales.length;i++) {
            	         if(festivosTotales[i].equals(FechaProcT2)) {
            	        	 if(fechaVencimientoT2.get(Calendar.DAY_OF_WEEK)==Calendar.FRIDAY) {
            	        		 fechaVencimientoT2.add(Calendar.DAY_OF_MONTH, 3);
            	        	 }else {
            	        	 fechaVencimientoT2.add(Calendar.DAY_OF_MONTH, 1);}
            	        	 //diasRestar = diasRestar + 1;
            	        	 ...
            	        	 if(fechaVencimientoT2.get(Calendar.DAY_OF_WEEK)==Calendar.FRIDAY&&habil==0) {
            	        		 fechaVencimientoT2.add(Calendar.DAY_OF_MONTH, 3);
            	        	 }else if(habil ==0){
            	        	 fechaVencimientoT2.add(Calendar.DAY_OF_MONTH, 1);
            	        	 }
